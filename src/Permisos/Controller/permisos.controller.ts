  ...
  async create(@Body() createPermisoDto: CreatePermisoDto) {
    try {
      if (!createPermisoDto.permiso?.trim()) {
        throw new HttpException('El nombre del permiso es requerido', HttpStatus.BAD_REQUEST);
      }

      if (!createPermisoDto.descripcion?.trim()) {
        throw new HttpException('La descripción del permiso es requerida', HttpStatus.BAD_REQUEST);
      }
